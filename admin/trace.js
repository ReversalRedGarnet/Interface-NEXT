 ...
 *
 * Otherwise self-contained — its only import is the shared click-trace
 * mechanic (see below), never anything room/campus-data-shaped. This is
 ...
 * itself, so there is nothing else here worth coupling to app code for.
 *
 ...
 *
 * The click-trace mechanic itself (pixel-coordinate conversion, the photo
 * decode step, and polygon rendering) lives in ../js/photo-trace.js, shared
 * with the editor's own "Trace from Photo" New Room option — this file only
 * adds what's specific to tracing a *campus* (the building-name/floor-count
 * prompts and the campus.json-shaped export below), same split as any other
 * "shared mechanic vs. this page's own use of it" module in this project.
 */
import { toImageCoords, loadPhotoFile, drawPolygon, boundingBox } from '../js/photo-trace.js';
...
canvas.addEventListener('click', evt => {
  if (!image) return;
  currentPoints.push(toImageCoords(canvas, evt.clientX, evt.clientY));
  updatePointsUI();
  redraw();
});
...
function redraw() {
  ...
  buildings.forEach(b => {
    drawPolygon(ctx, canvas.width, b.points, { stroke: '#4da3ff', fill: 'rgba(77,163,255,0.15)', closed: true });
    drawBoundingBox(boundingBox(b.points), '#ffb84d');
  });

  // In-progress shape.
  if (currentPoints.length) {
    drawPolygon(ctx, canvas.width, currentPoints, { stroke: '#4dff88', fill: currentPoints.length >= 3 ? 'rgba(77,255,136,0.15)' : null, closed: false });
  }
}
